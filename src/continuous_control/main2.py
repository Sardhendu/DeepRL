...
class Config:
    ...
    ACTOR_NETWORK_FN = lambda: Actor(Config.ACTION_SIZE, Config.STATE_SIZE, (512, 256), seed=2).to(
        device)  # lambda: Actor(Config.STATE_SIZE, Config.ACTION_SIZE, seed=2, fc1_units=512, fc2_units=256).to(device)
...
def ddpg(env, agent, brain_name, n_episodes=5000, max_t=2000):
    all_scores = []
...
class DDPG:
    def __init__(self, args, env, env_type='single'):
        self.env = env
    ...
    def ddpg(self, n_episodes=5000, max_t=2000):
        all_scores = []
        scores_window = deque(maxlen=100)
        NUM_AGENTS = 20
        running_time_step = 1
...
ddpg(env, agent, brain_name, n_episodes=5000, max_t=2000)
